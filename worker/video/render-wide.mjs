...
const C = { bg: "#090C12", surf: "rgba(21,27,39,0.82)", txt: "#EEF1F6", muted: "#A6AFBF", faint: "#818B9C", acc: "#CBFF46", acc2: "#8B6BFF", red: "#FF3355", ink: "#0B0E14" };
...
export async function renderWide({ game, codes, activeCount, fetchedAt, iconPath, outPath, voPath = null, music = true, sfx = true }) {
  ...
  const durAdegan = (list) => {
    const maxLen = Math.max(...list.map((c) => c.code.length), 8);
    return clamp(2.6 + maxLen / KETIK + 1.5, 4.2, 6.4);
  };
  const semua = [];
  ...
  function intro(ctx, ts) {
    ...
    if (s1 > 0) {
      ...
      ctx.save(); ctx.font = "700 54px Grotesk"; ctx.textAlign = "left";
      const fw = ctx.measureText(txt).width, left = W / 2 - fw / 2, yy = H / 2 + 34;
      const nn = Math.round(s1 * txt.length), sub = txt.slice(0, nn);
      pop(ctx, sub, left, yy, C.muted, 8);
      if (s1 < 1 && Math.floor(ts * 3) % 2 === 0) { ctx.fillStyle = C.acc; ctx.fillRect(left + ctx.measureText(sub).width + 5, yy - 26, 5, 48); }
      ctx.restore();
    }
  ...
  function adegan(ctx, list, ts, idxHal, gt) {
    ...
    const TB = 252, GAP = 36, y0 = 268;
    list.forEach((c, i) => {
      const mulai = 0.32 + i * 0.34;
      ...
      if (c.isNew) {
        ...
        rr(ctx, x0 + geser + lebar - tw - 32, y + 30, tw, 48, 24);
        ctx.fillStyle = "rgba(203,255,70,0.18)"; ctx.fill();
        ctx.fillStyle = C.acc; ctx.textBaseline = "middle"; ctx.fillText("BARU", x0 + geser + lebar - tw - 14, y + 55); ctx.textBaseline = "alphabetic";
      }
